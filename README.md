...
# 2. Bootstrap against your target repo (bare-clones it, creates base worktree, generates hub.config.json)
.\init-hub.ps1 -CloneUrl https://github.com/<owner>/<repo>.git

# 3. Edit hub.config.json: set installCmd, verifyCmd, testCmd; configure database.* if needed
#    Put real secrets in main\.env (copied into new worktrees automatically)

# 4. Initialize the SQLite ledger (one-time)
.\review-coverage.ps1 init
.\review-coverage.ps1 seed

# 5. Provision a solver worktree for a GitHub issue
.\new-worktree.ps1 -Issue 123 -Install

# 6. Launch the agent session (from inside the worktree)
cd issue-123-<slug>
...
All hub-specific values live in `hub.config.json` (git-ignored; generated by `init-hub.ps1` from `hub.config.example.json`). Edit it after bootstrapping.

| Field | Type | Default | What it controls |
|---|---|---|---|
| `repo` | string | *(required)* | `owner/repo` slug — passed to `gh` for issue lookups, PR creation, and recon |
| `cloneUrl` | string | *(required)* | HTTPS clone URL used by `init-hub.ps1` |
| `defaultBranch` | string | `"main"` | The repo's default branch; solvers base PRs off this; `retire-worktree.ps1` protects it |
| `baseWorktree` | string | `"main"` | Folder name of the base worktree (created by `init-hub.ps1`); source of `.env` files copied to new worktrees |
...
| `verifyCmd` | string | `"pnpm verify"` | Command agents run to validate before opening a PR (typecheck + lint) |
